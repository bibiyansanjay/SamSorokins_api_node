  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      return res.status(400).json({
        success: false,
        message: "Category.xlsx not found at /uploadCsv/category/",
      });
    }
    ...
    const cols = ["name", "abbreviation", "isActive"];
    const sql = `
    ...
    try {
      ...
      return res.status(200).json({
        success: true,
        message: `Inserted ${insertResult.affectedRows} categories successfully`,
      });
    } catch (e) {
      //await conn.rollback();
