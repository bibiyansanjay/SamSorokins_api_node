...
cron.schedule("0 * * * *", async () => {
  try {
    console.log("[UploadReminder] Running cron job to check for stale uploads...");
    ...
    console.log(`[UploadReminder] Found ${staleUploads.length} stale uploads. Process sending emails...`);
    ...
    for (const upload of staleUploads) {
      if (upload.userId && upload.userId.email) {
        ...
        console.log(`[UploadReminder] Sent email to ${upload.userId.email} for file ${upload.filename}`);
