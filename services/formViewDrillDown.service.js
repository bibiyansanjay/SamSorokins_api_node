...
async function applyExtraFilters(baseSql, filters) {
  let whereParts = [];
...
export async function getTrend(filters) {
  ...
  if (suffix === "form") {
    ...
    const { sql } = await applyExtraFilters(baseSql, filters);
    //console.log(sql);
    const [rows] = await pool.query(sql, [segment_id, company_id, form_id]);
    //console.log(rows);
    return rows;
...
export async function getResponseRateNResponsesCount(filters) {
  ...
  if (suffix === "form") {
    ...
    async function getSentCount(form_id, segment_id, company_id, filters) {
      ...
      const { sql } = await applyExtraFilters(baseSql1, filters);
...
export async function part3getRagDelta(filters) {
  ...
  if (suffix === "form") {
    ...
    const { sql } = await applyExtraFilters(baseSql, filters);
    //console.log(sql);
    const [rows] = await pool.query(sql, [form_id, form_id]);
    //console.log(rows);
    return rows;
  } else if (suffix === "form_wave") {
    const form_id = params[0];
