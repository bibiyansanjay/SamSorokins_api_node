...
export async function getTrend(filters) {
  ...
  if (suffix === "respondent") {
    ...
    const baseSql = `WITH base AS (
                ...
                WHERE wave_id = ?`;
    //console.log(baseSql);
    const [rows] = await pool.query(baseSql, [respondent_id, wave_id]);
    //console.log(rows);
    return rows;
...
/** 2) getAllSegments */
export async function getAllSegments(clientUserId) {
  const sql = `SELECT s.name as label, s.id as value
                FROM client_segment_users csu
                LEFT JOIN segment AS s ON s.id=csu.segment_id
...
/** 3) getAllWaves */
export async function getAllWaves(clientUserId) {
