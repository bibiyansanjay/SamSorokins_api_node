...
export async function getDateRange(filters) {
  ...
  if (suffix === "respondent") {
    ...
    const sql = `WITH base AS (
                        ...
                        WHERE w.wave_id = ?`;
    //console.log(sql);
    const [rows] = await pool.query(sql, [respondent_id, wave_id]);
    //console.log(rows);
    return rows;
  }
}
...
export async function getRagScore(filters) {
  ...
  if (suffix == "respondent") {
    ...
    const sql = `
              SELECT ROUND(AVG(option_numeric), 2) AS rag_score 
...
export async function getTop3Scores(filters) {
  ...
  if (suffix == "respondent") {
    ...
    const baseSql = `WITH base AS (
                ...
                `;
    //console.log(baseSql);
    const [rows] = await pool.query(baseSql, [respondent_id]);
    //console.log(rows);
    return rows;
  } else if (suffix == "respondent_wave") {
    const respondent_id = params[0];
...
/** 6) getFormsCompleted */
export async function getFormsCompleted(filters) {
  const { suffix, params } = pickVariant(filters);
  //
  if (suffix == "respondent") {
    const respondent_id = params[0];
    ...
    const sql = `SELECT 
                    respondent_id,
                    COUNT(DISTINCT CONCAT(form_id, '-', wave_id)) AS total_submissions
                  FROM form_data_company${company}
                  WHERE respondent_id = ?
    ...
    const sql = `SELECT 
                    ...
                    COUNT(DISTINCT CONCAT(form_id, '-', wave_id)) AS total_submissions
                  FROM form_data_company${company}
                  WHERE respondent_id = ? AND wave_id = ?
                  GROUP BY respondent_id;
            `;
    //console.log(sql);
    const [rows] = await pool.query(sql, [respondent_id, wave_id]);
...
export async function getCriticalIssues(filters) {
  ...
  if (suffix == "respondent") {
    ...
    const baseSql = `
                ...
                `;
    //console.log(baseSql);
    const [rows] = await pool.query(baseSql, [respondent_id, company_id]);
    //console.log(rows);
    return rows;
  } else if (suffix == "respondent_wave") {
    const respondent_id = params[0];
...
/* All Forms IDs */
async function getAllFormIdsNData(filters) {
  const { suffix, params } = pickVariant(filters);
  if (suffix == "respondent") {
    const respondent_id = params[0];
    const company = params[1];
    const sql = `SELECT 
                  ...
                  LEFT JOIN user_stakeholder_form usf ON fdc.form_id = usf.id
...
export async function getAllForms(filters) {
  const formData = await getAllFormIdsNData(filters);
...
/** 1) Respondent Meta */
export async function getRespondentMeta(clientUserId) {
  //console.log("clientUserId", clientUserId);
  const sql = `
            SELECT u.id, u.name, u.email, u.image, u.role, u.organization, u.city, u.region, u.country, u.stakeholder_id, stk.name as stakeholder_name, stk.image as stakeholder_image, stk.image_name as stakeholder_image_name, u.company,
            CONCAT(
...
/** 3) All Waves */
export async function getAllWaves(respondent_id, company) {
  const sql = `SELECT DISTINCT CONCAT("Wave ",wave_id)as label, wave_id as value FROM \`form_data_company${company}\` where respondent_id = ? ORDER BY wave_id ASC`;
  //console.log(sql);
  const [rows] = await pool.query(sql, [respondent_id]);
  return rows;
}
...
/** 3) All Segments */
export async function getAllSegments(respondent_id) {
  const sql = `
                  SELECT 
                         s.name as label,
                         s.id as value
