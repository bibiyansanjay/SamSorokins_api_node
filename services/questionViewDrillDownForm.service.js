...
function pickVariant({
  ...
  if (form_id && !wave_id && !segment_id && company_id && question_id)
    return {
      suffix: "form",
      params: [form_id, company_id, question_id, city, region, country],
    };
  if (form_id && wave_id && !segment_id && company_id && question_id)
    return {
      suffix: "form_wave",
...
export async function getTrend(filters) {
  ...
  if (suffix === "form") {
    ...
    const baseSql = `WITH base AS (
                ...
                SELECT wave_id, avg_score, delta_vs_prev_avg -- form_id 
                FROM scored
...
export async function getRagScoreWave1(filters) {
  const { suffix, params } = pickVariant(filters);
  //
  if (suffix == "form") {
    const form_id = params[0];
...
export async function getResponseRateNResponsesCount(filters) {
  ...
  if (suffix === "form") {
    ...
    const company_id = params[1];
    return Promise.all([
      getSentCount(form_id, filters),
      getResponsesCount(form_id, company_id, filters),
    ...
    async function getResponsesCount(form_id, company_id, filters) {
      const baseSql = `WITH base AS (
           SELECT
              fdc.form_id,
              fdc.wave_id,
    ...
    async function getSentCount(form_id, filters) {
      const baseSql = `
      ...
      const tableStr = `cu`;
      const { sql } = await applyExtraFilters(baseSql, filters, tableStr);
      const [sentRows] = await pool.query(sql, [form_id]);
    ...
    const company_id = params[2];
    return Promise.all([
      getSentCount(form_id, wave_id, filters),
      getResponsesCount(form_id, wave_id, company_id, filters),
    ]).then(([sentCount, responsesCount]) => {
      console.log("sentCount:", sentCount, "responsesCount:", responsesCount);
      // Calculate percentage
      const percentage =
    ...
    async function getResponsesCount(form_id, wave_id, company_id, filters) {
      const baseSql = `WITH base AS (
           SELECT
              fdc.form_id,
              fdc.wave_id,
    ...
    async function getSentCount(form_id, wave_id, filters) {
      const baseSql = `
              SELECT COUNT(*) AS count_occurrences
            FROM (
                SELECT
      ...
      const tableStr = `cu`;
      const { sql } = await applyExtraFilters(baseSql, filters, tableStr);
      const [sentRows] = await pool.query(sql, [form_id, wave_id]);
      const sentCount = sentRows?.[0]?.count_occurrences;
      return sentCount;
    }
  } else if (suffix === "form_segment") {
    ...
    const company_id = params[2];
    return Promise.all([
      getSentCount(form_id, segment_id, company_id, filters),
      getResponsesCount(form_id, segment_id, company_id, filters),
    ]).then(([sentCount, responsesCount]) => {
      console.log("sentCount:", sentCount, "responsesCount:", responsesCount);
      // Calculate percentage
      const percentage =
    ...
    async function getResponsesCount(form_id, segment_id, company_id, filters) {
      const baseSql = `WITH base AS (
           SELECT
              fdc.form_id,
              fdc.wave_id,
    ...
    async function getSentCount(form_id, segment_id, company_id, filters) {
      const baseSql1 = `
              SELECT COUNT(*) AS count_occurrences
            FROM (
                SELECT
    ...
    const company_id = params[3];
    return Promise.all([
      getSentCount(form_id, wave_id, segment_id, company_id, filters),
      getResponsesCount(form_id, wave_id, segment_id, company_id, filters),
    ]).then(([sentCount, responsesCount]) => {
      console.log("sentCount:", sentCount, "responsesCount:", responsesCount);
      // Calculate percentage
      const percentage =
...
export async function getWavePercentages(filters) {
  const { suffix, params } = pickVariant(filters);
  //
  if (suffix === "form") {
    const form_id = params[0];
...
//////////////////////////FORM/////////////////////////////////////////////-ENDS
//DONT APPLY WAVE FILTER HERE, AS WE WANT ALL WAVES DATA TO SHOW IN CHART N TABLE FRONTEND WILL AUTOMATICALLY TRUNCATE WAVES
//1111111111111111111111111111
//get date range for all data
export async function part1GetDateRange(form_id, company_id) {
  const sql = `WITH base AS (
                          SELECT
                            fdc.form_id,
                            fdc.wave_id,
...
export async function part3getRagDelta(filters) {
  ...
  if (suffix === "form") {
    ...
    const { sql } = await applyExtraFilters(baseSql, filters, tableStr);
    //console.log(sql);
    const [rows] = await pool.query(sql, [form_id, form_id, question_id]);
    //console.log(rows);
    return rows;
  } else if (suffix === "form_wave") {
    const form_id = params[0];
...
export async function part5getResponseRateNResponsesCount(filters) {
  ...
  if (suffix === "form") {
    ...
    async function getResponsesCount(form_id, company_id, filters) {
      ...
      const tableStr = `cu`;
      const { sql } = await applyExtraFilters(baseSql, filters, tableStr);
      const [responsesRows] = await pool.query(sql, [form_id, form_id]);
      //console.log(responsesRows);
    ...
    async function getSentCount(form_id, filters) {
      ...
      const tableStr = `cu`;
      const { sql } = await applyExtraFilters(baseSql, filters, tableStr);
      const [sentRows] = await pool.query(sql, [form_id, form_id]);
      return sentRows;
    }
  } else if (suffix == "form_wave") {
    const form_id = params[0];
